...
define(function(require, exports) {
	...
	$.VUI.widgetSetup('validator', {			
		rules: {
			...
			phone: [/^(((0\d{2,3})-)?(\d{7,8}))|(1\d{10})$/, LANG.PHONE], // 任意电话号码
            ...
            username: [/^\w{3,12}$/, LANG.USERNAME], // 用户名
		...
		// Use popover to show message
		showMsg: function(e, opts) {
			var $target = $(e.target), instance, 
				fullName = this.namespace + '-popover';
			switch (opts.type) {
				case 'error':
					instance = $target.data(fullName);
					opts = {
						content: opts.msg, 
						type: opts.type, 
						event: 'manual'
					};
					return instance ? instance.update(opts) : $target.popover(opts).data(fullName);
				default:
					return this._showMsg(e, opts);
			}
		}
	...
	function range($element, lvalue, rvalue) {
		var value = $element.val();
		return !rvalue ? lvalue === value : lvalue <= value <= rvalue;
	}
	function checked($element, lvalue, rvalue) {
		var value = this.$('input[name=' + $element.attr('name') + ']').filter(':checked').length;
		return !rvalue ? lvalue === value : lvalue <= value <= rvalue;
	}
	function length($element, lvalue, rvalue, x2) {
		var value = $element.val();
	...
	function match($element, delimiter, field) {
		...
		switch (delimiter) {
			case 'eq': 
				return (rvalue === value) || ('{0}与' + name +'不一致');
			case 'lt':
